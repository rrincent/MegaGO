...
env_scenarios <- NULL
env_scenarios$Experiment <- colnames(pheno_mat_Y_megalmm)
env_scenarios$Scenario <- replicate(1,createFolds(rnorm(env_scenarios$Experiment),k=5,list = F,returnTrain = F))
...
#--------------------------------------------------------------------------------------------------
sel_group=1 #if nreps are set to more than 1. The number 1 will represent rep1, 2 will be for rep2 and so on.

sel_trials <- env_scenarios$Experiment[which(env_scenarios$Scenario==sel_group)]

for (i in 1:nfolds) {
  
  pheno <- pheno_mat_Y_megalmm #change it to pheno_mat_Y_megaGO to run megaGO or megaGAO
  pheno[which(folds==i),sel_trials]=NA # it masks the data for test set.
  ...
  gblup_cors_pearson[[i]] <- diag(cor(pheno_mat_Y_megalmm[which(folds==i),sel_trials],
                                      GBLUP_predictions[which(folds==i),sel_trials],use = "p"))
  ...
  U_hat_cors_pearson[[i]] = diag(cor(pheno_mat_Y_megalmm[which(folds==i),sel_trials]
                                     ,U_hat[which(folds==i),sel_trials],use='p'))
  rownames(Eta_mean) <- rownames(U_hat)
  Eta_mean_sel <- Eta_mean
  rownames(Eta_mean_sel) <- gsub("::GENOTYPE","",rownames(Eta_mean_sel))
  
  
  Eta_mean_cors_pearson[[i]] = diag(cor(pheno_mat_Y_megalmm[which(folds==i),sel_trials]
                                        ,Eta_mean[which(folds==i),sel_trials],use='p'))
